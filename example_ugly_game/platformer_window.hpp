...
public:
    ...
    // и служит тактом сессии — и одиночному прогону, и сетевому. `false` — поверхность или устройство
    // потеряны насовсем, причина напечатана, сессия выходит ненулевым кодом (аудит #21 A·3·11).
    [[nodiscard]] bool draw(const Stage& stage);
