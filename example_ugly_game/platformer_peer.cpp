...
// Итераций без единого подтверждения, после которых пир перестаёт ждать дыру. Заведомо длиннее
// любого окна глухоты гейта 7: молчание собеседника лечится ожиданием, а форсированный тик — это
// отказ от сходимости, и путать их нельзя.
constexpr uint32_t STALL_LIMIT = 2000;
...
//
// Считается РАЗ ЗА ПРОХОД внешнего цикла, а не за шаг: внутри догоняющего цикла тот же счётчик
// выбрал бы свой потолок мгновенно и объявил бы дырой обычную перестановку датаграмм.
void give_up_on_a_hole(Peer& p) {
    if (p.tick < p.known + PEER_PREDICT || p.highest <= p.known) {
        p.stalled = 0;
        return;
    }
    if (++p.stalled < STALL_LIMIT) return;
    p.known = p.highest;
    advance_known(p);
    ++p.forced;
    p.stalled = 0;
