...
struct Peer {
    ...
    uint32_t queued = 0;
    uint32_t forced = 0;
    // Ввод, УЖЕ взятый у живой половины, но ещё не уехавший: место в окне отправки кончается
    // после того, как шов спросили, и второй раз спрашивать его на тот же тик нельзя. У скрипта
    // повтор бесплатен (чистая функция от номера тика), у человека — нет: чтение съедает фронты
    // клавиш этого кадра, и повторный вопрос вернул бы пустой ввод вместо нажатого.
    ch::MoveInput held{};
    bool holding = false;
    // Контроль гейта 9 подставляет отказ отправки РОВНО ОДИН РАЗ: отказывай он на каждом проходе,
    // тик не уехал бы никогда и прогон ушёл бы по дедлайну, ничего не сказав про кэш.
