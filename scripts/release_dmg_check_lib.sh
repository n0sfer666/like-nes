...
# каталог, из которого паковали. Подмена между раскладкой тома и hdiutil иначе прошла бы мимо.
assert_dmg_matches() {
  local mnt="$1" manifest="$2" got
  got=$(manifest_of "$mnt")
  if [ "$got" != "$(cat "$manifest")" ]; then
    bad "содержимое образа разошлось с манифестом"
    diff <(cat "$manifest") <(printf '%s\n' "$got") | sed 's/^/       /' >&2 || true
    return 1
  fi
  ok "образ совпал со своим манифестом"
}
