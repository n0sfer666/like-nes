...
# То же, что assert_pkg_matches у архива: гейт обязан утверждать про то, что УЕЗЖАЕТ, а не про
# каталог, из которого паковали. Подмена между раскладкой и упаковщиком иначе прошла бы мимо — и это
# верно для обоих установщиков одинаково, поэтому утверждение живёт здесь, а не двумя копиями в
# формато-специфичных библиотеках: разъехавшись, они разъехались бы молча.
assert_dir_matches() {
  local dir="$1" manifest="$2" got
  got=$(manifest_of "$dir")
  if [ "$got" != "$(cat "$manifest")" ]; then
    bad "содержимое пакета разошлось с манифестом"
    diff <(cat "$manifest") <(printf '%s\n' "$got") | sed 's/^/       /' >&2 || true
    return 1
  fi
  ok "содержимое совпало со своим манифестом"
}
