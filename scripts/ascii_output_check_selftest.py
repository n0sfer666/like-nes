...
FIXTURES = [
    ...
    # Маркер ищется в комментариях, а не в строках файла: до того как разбор забрали у
    # `cpp_text.marker_lines`, отписка, написанная ВНУТРИ литерала, подавляла находку — тот же
    # класс, что у двух швов закрыт с рождения.
    ("плохо: маркер внутри литерала не подавляет",
     'puts("привет"); puts("ascii: allow reason in three words");', 1),
]
