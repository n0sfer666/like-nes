...
Две формы эталона, потому что расходятся они по-разному:
  ...
  `mirrors-target` отбивал бы его ложно. Копий у него ЧЕТЫРЕ: `PLAT_FS` в трёх шагах (плагины,
  достижения и «Asset — ASan/UBSan», последняя — с аудита #21) и `TILES_FS` в четвёртом. Сверял их
  до 2026-08-24 никто: красная сборка того же дня — `undefined reference to platform::root_len` —
  это ровно расхождение одной копии с остальными.
