...
# пропускается вслух, и набор проверял бы только фикстуры.
stage "Примеры документации — самопроверка правил" \
    bash scripts/check_docs_examples_selftest.sh build-full
stage "Примеры документации собираются и печатают свои голдены (#19)" \
    bash scripts/check_docs_examples.sh build-full
