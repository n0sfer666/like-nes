...
say "  #16 гейт 7 ОТКРЫТ наполовину — счётчики закрыл CI, ВРЕМЯ из этапа выше судит эта машина; macOS и Linux замерены, осталась Windows"
say "  #17 гейт 9 ОТКРЫТ — образцы после переезда на графику: две записи против двух «до» (§7 руководства)"
say "  закрытые (#13 гейты 6 и 8, #14 гейт 8, #15 гейт 8, #16 гейт 8) — перепрогон, когда коммит тронул их поверхность"
