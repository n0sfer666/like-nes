...
# ничего: иначе `~` встал бы перед каждым путём. `BINMODE=3` — для gawk под git-bash: в текстовом
# режиме он съедает `\r` при чтении, и строки вывода MSVC теряли бы его в отчёте; прочие awk
# видят в нём обычную переменную. Сбой маскировки говорит об этом вслух: молча оставленный отчёт
# с домом уехал бы из машины как замаскированный.
redact_home_forms() {
    local h=${HOME:-}
...
redact_home_file() {
    ...
    RH_FORMS=$forms RH_CI=$ci awk -v BINMODE=3 '
