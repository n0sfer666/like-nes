...
bool fail(PresetBakeError& err, int line, const std::string& message) {
    err.line = line;
    err.message = message;
    return false;
}
...
bool need_preset(const PresetBuild& b, PresetBakeError& err, int line, const char* what) {
    ...
    return fail(err, line, std::string(what) + " before any preset");
}

bool parse_preset(PresetBuild& b, const std::vector<std::string>& f, int line, PresetBakeError& err) {
    if (f.size() != 2 || f[1].empty()) return fail(err, line, "preset needs exactly one name");
...
bool parse_action(PresetBuild& b, const std::vector<std::string>& f, int line, PresetBakeError& err) {
    ...
    if (f.size() < 3) return fail(err, line, "action needs a name and at least one source");
    ...
    for (uint32_t i = 0; i < cur.action_count; ++i)
        if (std::strcmp(b.blob.data.data() + b.actions[cur.action_begin + i].name_offset,
                        f[1].c_str()) == 0)
            return fail(err, line, "action '" + f[1] + "' is already declared in this preset; "
    ...
    for (std::size_t i = 2; i < f.size(); ++i) {
        ::input::Source src;
        if (!parse_source(f[i], src)) return fail(err, line, "unknown source '" + f[i] + "'");
...
bool parse_axis(PresetBuild& b, const std::vector<std::string>& f, int line, PresetBakeError& err) {
    ...
    if (f.size() != 4)
        return fail(err, line, "axis needs a name, a positive source and a negative source ('-' for none)");
    ::input::Source pos{}, neg{};
    if (!parse_source(f[2], pos)) return fail(err, line, "unknown source '" + f[2] + "'");
    if (f[3] != "-" && !parse_source(f[3], neg)) return fail(err, line, "unknown source '" + f[3] + "'");
    AxisRow ax{};
...
bool parse_shape(PresetBuild& b, const std::vector<std::string>& f, int line, PresetBakeError& err) {
    ...
    fix32 dz, outer;
    if (!parse_fix(f[2], dz) || !parse_fix(f[3], outer))
        return fail(err, line, "deadzone and outer edge must be decimal numbers");
    PresetShape s;
    s.deadzone_raw = dz.raw;
    s.outer_raw = outer.raw;
    if (!parse_small_int(f[4], s.curve_exp) || s.curve_exp < 1 || s.curve_exp > 4)
        return fail(err, line, "curve exponent must be 1..4");
    if (f[5] != "-") s.pair = f[5];
    b.shapes[f[1]] = s;
    return true;
}
...
bool preset_parse_line(PresetBuild& b, const std::vector<std::string>& f, int line,
    ...
    return fail(err, line, "unknown row kind '" + kind + "'");
}
