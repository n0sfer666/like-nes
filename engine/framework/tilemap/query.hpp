...
// добавила односторонние и склоны: свип персонажа вниз обязан видеть односторонний тайл, а свип
// вверх — нет, и это разница В ЗАПРОСЕ, а не в карте.
//
// `exclude` выключает тайл ЦЕЛИКОМ, что бы он ни нёс, и заведён он ради спуска по команде: тайл
// `solid oneway` проходит `require = TILE_SOLID`, поэтому убрать его из запроса маской требования
// нечем. Тик спуска ставит `exclude = TILE_ONEWAY` — один тик, а дальше платформа перестаёт держать
// сама, потому что ноги персонажа уже ниже её верха.
struct TileFilter {
    TileFlags require = TILE_SOLID;
    TileFlags exclude = TILE_EMPTY;
};
...
// ОДНОСТОРОННИЙ ТАЙЛ (`TILE_ONEWAY`) участвует во всех трёх запросах наравне с прочими — вопрос
// «кто здесь» у него тот же, — но КАСАНИЕ от него получает не всякий: свип принимает его хит,
// только если грань смотрит вверх (`normal.y < 0`) и низ формы на СТАРТЕ пути был не ниже верха
// тайла. Первое условие снимает боковые грани и низ, второе — того, кто пришёл снизу или уже
// провалился внутрь; вместе они дают «проход снизу, опора сверху» без единого бита в карте про
// направление. `overlap_shape` этих условий не знает вовсе: у него нет пути, а значит нет и того,
// откуда пришли, — он отвечает на «кто здесь СЕЙЧАС», и односторонний тайл здесь.
