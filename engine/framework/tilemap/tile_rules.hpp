...
//
// Радиус входит в ответ, а не отбрасывается: ядро круга — ОДНА точка, и `sanitize` выдаёт радиус
// всякой форме с `count < 3` (`physics/shape.cpp`). Низ по одним вершинам у круга радиуса 4 врал бы
// ровно на эти 4 юнита, то есть провалившийся сквозь платформу мяч читался бы как пришедший сверху
// и его втащило бы обратно. Правило здесь — правило ЗАПРОСА к сетке, а не персонажа: у коробки
// радиус ноль и ответ не меняется, а всякий другой потребитель свипа получал бы неверный молча.
