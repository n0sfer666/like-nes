...
// Перепечатан ОСОЗНАННО третий раз (`0xe243e1457a11f65e` → значение ниже) вместе с точным ответом
// `nearest_on_core` на длинной грани. Причина — «осознанная смена формулы», и старое значение здесь
// не эталон, а ЗАПИСАННЫЙ ДЕФЕКТ: точка на грани строилась умножением ребра на параметр в Q16.16,
// то есть с квантом `длина_ребра / 65536` вдоль грани (0.006 юнита на 400), а нормировалась
// разность, поперечная составляющая которой того же порядка — свип останавливается в `CONTACT_SLOP`
// = 0.0625 от поверхности. Нормаль уезжала на десятки градусов: персонаж, въехавший в тонкую стену
// на 2048 ю/с, получал (-0.14, -0.99) вместо (-1, 0) и улетал ВВЕРХ через неё. Дефект пинится
// собственным гейтом (`test_long_face_normal` в `framework_physics_point_test`), который на старой
// реализации падает, — иначе перепечатка была бы неотличима от подгонки константы под регресс.
// Эталон снят с прогона, где прошли все проверки поведения в этой цели и все остальные цели физики.
constexpr uint64_t GOLDEN = 0xf238d0bc34325db6ULL;
