...
public:
    ...
    //
    // Возвращает число рассмотренных кандидатов — проходов внутреннего цикла до отсечений. Это
    // мера, ради которой SAP и выбран: она отвечает на вопрос «а не перебираем ли мы каждого с
    // каждым», на который размер `out` ответить не может (см. `counters.hpp`). Возвращается, а не
    // копится в поле, потому что величина принадлежит ВЫЗОВУ: поле пришлось бы обнулять снаружи, и
    // забытое обнуление дало бы растущее число, которое гейт прочитал бы как деградацию.
    uint64_t build(const std::vector<Body>& bodies, std::vector<Pair>& out);
