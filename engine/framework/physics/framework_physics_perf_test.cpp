...
int main(int argc, char** argv) {
    ...
    // Куча перепечатана второй раз (`{350, 966, 5869, 966, 20480, 1065}` → ниже) вместе с точным
    // ответом `nearest_on_core` на длинной грани; разбор — у голдена в `framework_physics_test`.
    // Числа ушли ВНИЗ по всем пяти счётчикам, и направление само по себе довод: верная нормаль
    // укладывает кучу плотнее. `scatter` и `column` не задеты, что и ограничивает радиус.
    check_reference("heap", heap, Reference{350, 964, 5865, 964, 20304, 1052});
