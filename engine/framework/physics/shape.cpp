...
bool overlaps(const Aabb& a, const Aabb& b) {
    // Касание границы перекрытием НЕ считается: `<` вместо `<=`. Предикат отвечает про сами
    // прямоугольники и обязан быть строгим, иначе тела, разошедшиеся ровно на ноль, числились бы
    // пересекающимися. Расширять область поиска — не его дело: широкая фаза добавляет к границам
    // спекулятивное поле САМА (`broadphase.cpp`), и пара впритык доходит до узкой фазы через него.
