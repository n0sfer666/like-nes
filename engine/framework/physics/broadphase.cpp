...
// касание границ отсутствием перекрытия (строгое `<`), поэтому два тела, стоящие ровно впритык, не
// доходят до узкой фазы — пары просто нет. Именно так башня и проваливалась сама в себя: опоры нет,
// пока ящик в неё не влез.
//
// Расширение на ПОЛНОЕ поле с каждой стороны, а не на половину: широкая фаза обязана быть
// НАДМНОЖЕСТВОМ — пропущенная ею пара невосстановима, а лишняя стоит одного отказа узкой фазы.
Aabb padded(const Aabb& a, fix32 margin) {
