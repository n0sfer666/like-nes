...
void overlap_shape(const World& w, const Shape& s, Vec2 position, fix32 angle, const QueryFilter& f,
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(bodies.size()); ++i) {
        ...
        to_world(b.shape, b.position, b.rot, target);
        Manifold m;
        // Той же узкой фазой, что и шаг мира. Своя проверка пересечения здесь была бы вторым ответом
        // на тот же вопрос, и разошлись бы они на касании: игра, спросившая «стою ли я в зоне», и
        // решатель, разрешающий контакт с той же зоной, обязаны отвечать одинаково.
        if (!collide_shapes(probe, position, target, b.position, m)) continue;
