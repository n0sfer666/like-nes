...
void test_manifold() {
    ...
    Manifold m;
    check(collide(make_body(crate), ground, m), "box-box overlap detected");
    ...
    check(collide(make_body(crate), ground, m), "rotated box still collides");
    ...
    check(collide(make_body(cap), ground, m), "capsule-box overlap detected");
    ...
    check(collide(make_body(wedge), ground, m), "polygon-box overlap detected");
...
//
// Раскладка не придумана: перебор 95 млн пар (две формы, 32 угла каждой, сетка сдвигов) нашёл ровно
// две, и эта — вторая. Именно поэтому она выглядит произвольной: касание идёт по краю, глубина
// сотые доли юнита. До фикса пара уходила на путь ЗАЗОРА, который ищет минимум расстояния между
// множествами и на пересекающихся не находит ничего: он возвращал нормаль от вершины в восемнадцати
// юнитах — перпендикулярную настоящей — с нулевой глубиной. Тела при этом разъезжались поперёк.
void test_reference_axis() {
    ...
    a.angle = fix32::from_float(0.6875); // 247.5 градуса, в оборотах — точная двоичная дробь
    BodyDesc b;
    b.key = 2;
    b.shape = polygon(tri, 3);
    b.angle = fix32::from_float(0.1875); // 67.5 градуса
    b.position = {fix32::from_int(-16), fix32::from_int(-4)};

    const Body ba = make_body(a);
    const Body bb = make_body(b);
    Manifold m;
    check(collide(ba, bb, m), "control: cores touching by a sliver still report a contact");
