...
uint64_t solve_velocity(std::vector<Body>& bodies, std::vector<Manifold>& manifolds, fix32 dt) {
    ...
    for (uint32_t it = 0; it < VELOCITY_ITERATIONS; ++it) {
        for (Manifold& m : manifolds) {
            ...
            //
            // Баланс держится на чётности `VELOCITY_ITERATIONS`: половину итераций каждая точка
            // первая, половину последняя. Нечётное число вернуло бы перекос — поэтому число итераций
            // часть контракта (см. заголовок).
