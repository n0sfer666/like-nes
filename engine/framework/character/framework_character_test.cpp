...
//
// Перепечатан 2026-08-24 переездом сцены на ТАЙЛЫ (вертикаль 3): пол и стена выложены сеткой, а
// телом остался только потолок — так голден проходит и через слияние ответов двух источников
// (`collision.hpp`), а не только через каждый из них поодиночке. Геометрия при этом та же ЮНИТ В
// ЮНИТ, что у прежних трёх коробок, поэтому расхождение здесь измеряет РОВНО одно: насколько ответ
// сетки отличается от ответа мира тел на той же грани.
//
// Замерено сравнением двух прогонов, тик в тик: расходятся тики 2..408 из 600, максимум сдвига
// 0.0136 юнита по X и 0.0621 по Y — обе величины МЕНЬШЕ `physics::CONTACT_SLOP` (0.0625), то есть
// лежат внутри допуска, на котором свип и останавливается. Ни один дискретный признак не разошёлся
// НИ РАЗУ: `state`, `on_ground`, `hit_wall`, `hit_ceiling` совпали на всех 600 тиках, и конечная
// позиция совпала побитово. Сдвиг берётся оттуда, что пол из тайлов — это много коробок, и точка
// отхода на зазор считается от грани того тайла, который ответил.
//
// Перед этим (2026-08-23) голден перепечатывался с фиксом нормали свипа на вырожденном касании
// (`cast.cpp`, `terminal_normal`): свидетель расстояния сцеплял угол корпуса с углом пола и отдавал
// свипу нормаль (0.707, -0.707) вместо (0, -1), отход на зазор уносил персонажа вправо ОТ стены и
// поднимал его над полом на 0.055 юнита до конца фазы.
constexpr uint64_t GOLDEN = 0x7801049c168bb301ull;
...
void test_golden() {
    ...
    for (uint32_t t = 0; t < TICKS; ++t) {
        ...
        step(s, hull, p, d, scripted(t), tick_dt(), c);
