...
void step(const CollisionScene& s, const CharacterHull& hull, const MoveProfile& raw,
    ...
    // 0. Перенос опорой и снос движущимся телом — ДО всего остального: платформа уже сдвинулась
    // (порядок кадра «физика, потом персонаж»), а все восемь шагов ниже спрашивают геометрию, и
    // спрашивать её из положения, которого в мире не было ни мгновения, значит мерить полтика
    // назад. Перенос — только со СТОЯЩИМ: опора помнится и в окне coyote ради наследования
    // горизонтали, но сошедшего возить по воздуху нельзя. Снос — со всяким (`push.hpp`).
    c.crushed = !moved_by_world(s, hull, c.support, was_ground, dt, c.position);
