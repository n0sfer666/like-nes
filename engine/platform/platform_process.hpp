#pragma once
#include <cstdint>
#include <string>
#include <vector>

// Шов №2 платформы (спека #12, решение 2): запуск дочернего процесса. Реализации —
// platform_process_posix.cpp (fork/execvp) и platform_process_win32.cpp (CreateProcessW),
// выбор делает CMake, условной компиляции внутри нет.
//
// Потребители: пекарь (assetc дёргает пиннутые кодеки), build-loop и Play-spawn (#13).
namespace platform {

// Запускает argv[0] (поиск по PATH) с аргументами argv[1..], ждёт завершения.
// true — процесс завершился нормально с кодом возврата 0.
//
// stdout/stderr ребёнка уходят в /dev/null (NUL): инструменты пишут результат в файл, а их
// вывод только засоряет лог пекаря. Кому понадобится захват вывода — это отдельная функция,
// а не флаг: диагностики компилятора (#13) нужны построчно и с разбором.
bool run_tool(const std::vector<std::string>& argv);

// Идентификатор текущего процесса. Нужен там, где имя системного объекта обязано быть уникальным
// на машине (сегмент разделяемой памяти двух параллельных прогонов) — getpid() против
// GetCurrentProcessId(). Как число, а не строка: форматирование — дело вызывающего.
uint32_t process_id();

// Дочерний процесс, которым управляют, а не просто ждут. Нужен гейтам живучести: убить пишущего
// на середине транзакции и посмотреть, что осталось на диске.
//
// В интерфейс сознательно не вынесен fork: на Windows его нет, и «убить копию себя» пришлось бы
// эмулировать. Поэтому ребёнок здесь ВСЕГДА отдельный процесс по argv — тест перезапускает
// собственный exe (platform::exe_path) со служебным флагом.
// Как именно завершился ребёнок. Разница между «упал» и «убит» несущая: гейт крэш-изоляции
// доказывает ИМЕННО падение, а «убит» — штатный останов игры редактором.
//
// Отличать их приходится по-разному: на POSIX убийство и падение — разные сигналы, на Windows
// сигналов нет вовсе, и TerminateProcess задаёт обычный код возврата, неотличимый от exit(N).
// Поэтому «убит» помечает СЕБЯ убивший объект Child, а не разбор кода задним числом.
enum class ExitKind {
    Exited,   // вернулся из main / вызвал exit — code осмыслен
    Crashed,  // нарушение доступа, недопустимая инструкция, abort
    Killed,   // остановлен через kill_and_wait этого же объекта
    Unknown,  // ожидание не удалось: код возврата брать неоткуда
};

struct ExitStatus {
    ExitKind kind = ExitKind::Unknown;
    int code = -1;   // осмыслен при Exited; при Crashed — номер сигнала (POSIX) / код исключения
};

// Запускает argv и возвращает ВЕСЬ его вывод. stdout и stderr сведены в один поток намеренно:
// компиляторы печатают диагностики в stderr, а «в какой из потоков ушла строка» парсеру не нужно,
// зато чередование строк из двух потоков теряло бы порядок сообщений.
//
// Отдельная функция, а не флаг run_tool: захват требует канала и чтения до EOF, и совмещать это
// с «просто дождаться» значило бы платить каналом там, где вывод выбрасывается.
// false — процесс не запустился либо не дождались; output тогда содержит то, что успело прийти
// (разбирать его всё равно надо: пустая панель ошибок при провале читается как «ошибок нет»).
//
// Ждёт, пока вывод не дочитан до EOF, — на обеих ОС без таймаута: сколько идёт компиляция, знает
// сборка, а не шов. Расхождение ровно одно и намеренное: после EOF Windows отводит на само
// завершение 30 с (упавший процесс там способен неопределённо долго висеть в WerFault), POSIX
// ждёт waitpid столько, сколько нужно.
bool run_capture(const std::vector<std::string>& argv, std::string& output, ExitStatus& status);

class Child {
public:
    Child() = default;
    ~Child();
    Child(Child&& o) noexcept;
    Child& operator=(Child&& o) noexcept;
    Child(const Child&) = delete;
    Child& operator=(const Child&) = delete;

    bool spawn(const std::vector<std::string>& argv);
    bool alive() const { return raw_ != 0; }

    // Убить немедленно и дождаться. true — процесс умер ИМЕННО от убийства: успей он выйти сам,
    // гейт «убили на середине записи» был бы вакуумным, и отличать эти два исхода обязан шов.
    bool kill_and_wait();

    // Дождаться завершения САМОГО ребёнка, не убивая. Объект после возврата пуст (alive()==false):
    // ждать дважды нечего, а осиротевший pid/HANDLE — ровно то, из-за чего заводится зомби.
    // out сбрасывается в начале: false (ожидание не состоялось — ребёнка нет либо он не дождался)
    // обязан читаться как Unknown и в переиспользованной структуре, а не как исход прошлого ребёнка.
    bool wait(ExitStatus& out);

private:
    intptr_t raw_ = 0; // pid_t на POSIX, HANDLE на Windows
};

} // namespace platform
