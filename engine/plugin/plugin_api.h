...
/* Потолок на число зависимостей одной sim-системы. Часть КОНТРАКТА, а не внутреннее число хоста:
   `n_after` приходит из кадра плагина как `int32_t`, и без потолка хост читает `after[0..n)` по
   чужому счётчику — обычная опечатка в плагине превращается в чтение мусора, а в логе стоит адрес
   хоста, а не имя плагина. Шестьдесят четыре — того же порядка, что `MAX_BASE_DEPTH` у таблицы
   материалов: система с таким числом зависимостей уже ошибка проектирования, а не тесный лимит. */
#define PLUGIN_MAX_SYSTEM_DEPS 64

/* Потолок на РАЗМЕР БУФЕРА любой строки из кадра плагина: `id`, `fourcc`, заголовок панели, имя
   зависимости. Терминатор входит в это число — то есть содержимого не больше 127 байт, и хост
   читает ровно столько, сколько контракт разрешает подать. Тоже часть КОНТРАКТА: без него он
   написан наполовину — счётчик массива ограничен, а строка нет, хотя не-NUL-терминированный
   буфер заставляет хост читать чужую память до первого случайного нуля. Сто двадцать восемь взято
   по дереву с запасом: самый длинный id в нём — одиннадцать байт, а связывает не id, а заголовок
   панели для человека (двадцать девять). Тот же приём, что у имён пресетов ввода (`MAX_NAME`
   в preset_format.hpp). */
#define PLUGIN_MAX_ID 128
