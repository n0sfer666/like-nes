...
public:
    ...
    // Индекс ТЕКСТУРНОЙ строки по имени слота, с той же развёрткой базы. Нет такой — `-1`.
    // Слот адресуется именем, а не номером: номер `2` в проходе нормалей и номер `2` в
    // `library.mat` — два независимых числа, и совпадать они перестали бы молча, тусклым светом,
    // а не отказом (тот же класс, что закрывает `slot_of` для параметров).
    int32_t texture_of(uint32_t i, const char* slot_name) const;
