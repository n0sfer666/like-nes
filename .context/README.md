...
| 15 | Детерминированная 2D-физика — ядро | **В работе, вертикаль 1 из 3** (ADR 0015 Proposed; тела/круг/AABB, sweep-and-prune, импульсный решатель с фиксированным числом итераций. Гейты 1, 2, 6, 7, 9 закрыты машинно: golden состояния `0x3977752decf95c28` на 3 ОС / перетасованный порядок создания даёт тот же хеш + негативный контроль / ноль аллокаций в `step()` и `hash()` со счётчиком, проверенным реальной аллокацией / ASan+UBSan на `-O1` молчат и дают тот же golden, что `-O3` / `0x32a094e8…`, `0xe728fef1…`, `0x4169690…` целы. Плюс гейт на ГРАНИЦЫ заявленного диапазона отдельной целью — он и вскрыл насыщение импульса, которое голден из середины диапазона пропускал. Открыты гейты 3, 4, 5, 8 — вертикали 2 и 3) — [`specs/2026-07-26-physics-core.md`](specs/2026-07-26-physics-core.md) · [`decisions/2026-08-08-physics-core.md`](decisions/2026-08-08-physics-core.md) |
