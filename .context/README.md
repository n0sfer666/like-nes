...
| 13 | Паритет среды разработки на трёх десктопных ОС | **Закрыта частично** (ADR 0013 Proposed; гейты 1–5 и 7 зелёные на 3 ОС: shmem-зеркало, Play-spawn, build-loop с живым `cl.exe`, диагностики MSVC, перф, кросс-ОС сцена. Единственный `if(NOT WIN32)` в дереве — `ide_ipc_bench`, постоянная граница. **Гейты 6 (X11+Wayland) и 8 (сквозной сценарий) — ожидание проверки от разработчика** на его Windows- и Linux-машинах; до них ADR не Accepted) — [`specs/2026-07-26-desktop-dev-parity.md`](specs/2026-07-26-desktop-dev-parity.md) · [`decisions/2026-07-27-desktop-dev-parity.md`](decisions/2026-07-27-desktop-dev-parity.md) |
| 14 | Каркас gameplay-фреймворка + ввод из коробки | **Закрыта частично** (ADR 0014 Proposed, спека T4, гейты 1–7 машинно: `Schedule` с детерминированным порядком / fix32-форма стика (радиальная и по-осевая мёртвая зона, кривые, триггеры) / пресеты и профили падов — испечённые zero-parse данные в `game.bundle`, golden разрешения ввода `0x23d71ef8855fa5c1` на 3 ОС / профиль пада по VID/PID из нового шва HAL `pad_info`, неизвестный → generic, hotplug / перебинды накладкой по ИМЕНИ действия с конфликтом и force, транзакционная запись профиля / игра-образец переведена на пресеты, sim-golden `0x32a094e8…` цел / направление зависимостей подсистемы→слой запрещено грепом в CI / 0 аллокаций в тике. **Гейт 8 (owner-HW: пад, перебинд в рантайме, отключение на ходу) — ожидание проверки разработчиком.** Долг в ADR: `write_atomic` продублирован с достижениями) — [`specs/2026-07-26-framework-input.md`](specs/2026-07-26-framework-input.md) · [`decisions/2026-07-28-framework-input.md`](decisions/2026-07-28-framework-input.md) |
