...
## 2026-08-01: чёрный экран на Windows — как различать причины

Машина владельца: `[gpu] GeForce MX150 | Vulkan | BC: yes`, звук есть, `window clean exit`,
ошибок нет **ни одной** — ни `[wgpu] error`, ни пропуска поверхности. То есть кадры собираются,
сабмитятся и презентуются успешно, а на экран не попадают. Это гибридный ноутбук (Intel iGPU +
дискретная MX150): кадр рисуется на одной карте, окном владеет другая, composition между ними —
дело драйвера, и изнутри процесса его отказ неотличим от нормальной работы.

Две ручки для различения (обе — диагностика, не тюнинг):
- `scripts\win-dev.bat demo` — тот же пайплайн в PNG, без окна и без поверхности вовсе.
  Кадры нормальные → отрисовка цела, сломана презентация. Кадры чёрные → виноват движок.
- `scripts\win-dev.bat game d3d12` / `game low` — сменить бэкенд или увести адаптер на
  интегрированный (`LIKENES_GPU_BACKEND`, `LIKENES_GPU_POWER`).

**Грабля, найденная при реализации:** wgpu-native **игнорирует**
`WGPURequestAdapterOptions.backendType` и печатает в лог `... is unsupported, use
WGPUInstanceExtras.backends`. Ручка, поставленная там, выглядела бы рабочей и молча ничего не
делала — диагностический прогон дал бы ложное «смена бэкенда не помогла». Отбор бэкендов идёт
только на ИНСТАНСЕ (`WGPUInstanceExtras`, `GpuContext::create_instance`), а раз surface создаётся
до `init`, инстанс обязан создавать тот же метод. Проверено на macOS: `LIKENES_GPU_BACKEND=vulkan`
теперь честно отвечает `No suitable adapter found`, а не «успешно проигнорировано».

Ручки живут полями `GpuContext`, а окружение читает `example_ugly_game/gpu_env.cpp`:
`engine/render/gpu.cpp` компилируется НАПРЯМУЮ в шесть целей, включая iOS и Android, у которых нет
`platform_core` — попытка залинковать env-шов в render валит сборку `editor_shell` и
`plugin_ui_shell` и сломала бы мобильные сборки молча.

Сцена игры почти целиком чёрная (космос): признак «картинка есть» — цифры счёта в углах и
корабли, а не яркий фон.
