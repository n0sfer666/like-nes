...
(спека #11). Плюс столп №4 требует «все устройства из коробки» (Sony/MS/Logitech). Нужна архитектура,
...
- Даёт вводу тот же детерм.-контракт, что output-доменам (рендер #2 / аудио #3): единственный вход в
  сим — записываемый/воспроизводимый per-tick снапшот; **готовит netplay/rollback (спека #11)** —
