...
rollback и netplay (спека #11) держатся на том, что ввод сэмплится в фиксированной точке тика и
...
- **Гейт #1 (record→replay→sim-hash):** записанный `InputFrame`-поток → реплей даёт байт-идентичный
  sim-hash `0x4c6429af58bde01e`. **Сильнее:** тот же хеш при плотности синтетич. опроса d1≡d3≡d8 →
...
6. **Карта биндингов как ассет** (hot-reload через #5) + IDE-редактор перебиндов/dead-zone кривых —
   спека #7.
7. **Интеграция с netplay/rollback (#11):** `InputFrame`-поток = основа отправки инпутов по сети +
