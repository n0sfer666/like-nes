...
6. **Платформа входит только через шов `engine/platform/*`** (спека #12, ADR 0012): ввод-вывод
   ...
   спека #22), именованная разделяемая память (`platform_shmem`, спека #13) и наблюдение за
   каталогом (`platform_watch`, спека #13). Реализацию выбирает CMake парой
   `*_posix.cpp`/`*_win32.cpp` — кроме `platform_watch`, у которого API один, а механизмов ОС
   три (`inotify`, `FSEvents`, `ReadDirectoryChangesW`) плюс опрос как запасной путь; условной
   компиляции по ОС вне шва нет — проверяется грепом в CI **до** сборки.
